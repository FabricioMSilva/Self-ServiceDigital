...
import { buildQuoteEstimate } from "../lib/quotePricing";
          ...
          <div className="catalog-question__options" role="group" aria-label="Itens desejados no projeto">
            {featuresQuestion.options.map((option) => (
              <button
                key={option.id}
                type="button"
                className={`catalog-question__option catalog-question__option--compact${
                  selectedFeatures.includes(option.id) ? " is-selected" : ""
                }`}
